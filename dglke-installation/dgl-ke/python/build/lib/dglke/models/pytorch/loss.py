...
class BaseMarginLoss(nn.Module):
    def __init__(self, margin):
        super(BaseMarginLoss, self).__init__()
        self.loss_fn = th.nn.MarginRankingLoss(margin=margin)

    def forward(self, pos_score, neg_score):
        # Assume label of 1 for positive ranking of pos_score over neg_score
        label = th.ones_like(pos_score)
        return self.loss_fn(pos_score, neg_score, label)

class MarginLoss(BaseMarginLoss):
    def __init__(self, margin):
...
class LossGenerator(BaseLossGenerator):
    def __init__(self, args, loss_genre='Logsigmoid', neg_adversarial_sampling=False, adversarial_temperature=1.0,
        ...
        elif loss_genre == 'Margin':
            self.neg_label = -1  # Set according to requirements
            self.loss_criterion = MarginLoss(args.margin)
        else:
