...
def l2_torus_dissimilarity(a_minus_b):
    tmp = a_minus_b.pow(2)
    return 4 * torch.min(tmp, 1 - tmp).sum(dim=-1)
...
model = TorusEModel(emb_dim, kg_train.n_ent, kg_train.n_rel, 'torus_L2')
...
def train():
  ...
  # Pos
  x = perform_spmm(adj1.to(device), all_emb)
  p = -1 * l2_torus_dissimilarity(x)

  # # Neg
  x_ = perform_spmm(adj1_neg.to(device), all_emb)
  n = -1 * l2_torus_dissimilarity(x_)
